...
def resolve_bunny(embed_url, session):
    ...
    if not m:
        raise RuntimeError(i18n.t("ui.stream_bunny", url=embed_url))
...
def resolve_pandavideo(embed_url, session):
    ...
    if not pullzone or not video_id:
        raise RuntimeError(i18n.t("ui.stream_panda", url=embed_url))
...
def resolve_scaleup(embed_url, session):
    ...
    if not m:
        raise RuntimeError(i18n.t("ui.stream_scaleup", url=embed_url))
...
def resolve_hotmart(embed_url, session):
    ...
    if not m:
        raise RuntimeError(i18n.t("ui.stream_hotmart", url=embed_url))
...
def resolve_memberkit_vimeo(embed_url, session):
    ...
    if not uid:
        raise RuntimeError(i18n.t("ui.stream_vimeo_url", url=embed_url))
    ...
    if not default_cdn:
        raise RuntimeError(i18n.t("ui.stream_vimeo_hls", url=embed_url))
    return {
        "kind": "ytdlp",
